...
namespace Mapbox.Unity.Map
{
	...
	public class AbstractMap : MonoBehaviour, IMap
	{
		...
		protected float _unityTileSize =10;
		...
		protected float _worldRelativeScale=10;
		...
		protected virtual void InitializeMap(MapOptions options)
		{
            ...
            options.locationOptions.latitudeLongitude = default_pos;
            
            _centerLatitudeLongitude = Conversions.StringToLatLon(options.locationOptions.latitudeLongitude);//original
        ...
        //author Ziyao He
        public virtual Vector3 GeoToWorldPositionXYZ(Vector2d latitudeLongitude, float height, bool queryHeight=true)
        {
            Vector3 worldPos = GeoToWorldPositionXZ(latitudeLongitude);
            ...
            if (queryHeight)
            {
                //Query Height.
                float tileScale = 1f;
                float geoHeight = QueryElevationAtInternal(latitudeLongitude, out tileScale);
